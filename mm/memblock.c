...
phys_addr_t g_hotplug_aligned_blocks[NUM_ALIGN_BLK];
		...
		while (addr + MIN_MEMORY_BLOCK_SIZE <= rgn_end) {
			if (!memblock_in_no_hotplug_area(addr)) {
				if (blk == NUM_ALIGN_BLK-1) {
					pr_err("%s: g_hotplug_aligned_blocks overflows\n", __func__);
					return 0;
				}
				g_hotplug_aligned_blocks[blk++] = addr;
...
int memblock_dump_aligned_blocks_addr(char *buf)
{
	...
	if (g_hotplug_aligned_blocks[idx]) {
		size += snprintf(buf+size, 32, "0x%llx", g_hotplug_aligned_blocks[idx]);
		idx++;
	}

	while (g_hotplug_aligned_blocks[idx]) {
		size += snprintf(buf+size, 32, ",0x%llx", g_hotplug_aligned_blocks[idx]);
		idx++;
	}
	return size;
...
int memblock_dump_aligned_blocks_num(char *buf)
{
	...
	if (g_hotplug_aligned_blocks[idx]) {
		size += snprintf(buf+size, 16, "%d",
			(int)(g_hotplug_aligned_blocks[idx] >> SECTION_SIZE_BITS));
		idx++;
	}
