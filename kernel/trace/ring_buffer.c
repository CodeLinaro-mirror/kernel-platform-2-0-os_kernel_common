...
void ring_buffer_poke(struct trace_buffer *buffer, int cpu)
{
	struct ring_buffer_per_cpu *cpu_buffer;

	if (cpu < 0 || cpu >= buffer->cpus)
		return;
...
size_t trace_buffer_pack_size(struct trace_buffer *trace_buffer)
{
	...
	for (i = 0; i < trace_buffer->cpus; i++) {
		struct ring_buffer_per_cpu *rb = trace_buffer->buffers[i];
...
void trace_buffer_pack(struct trace_buffer *trace_buffer,
		       struct trace_buffer_pack *pack)
{
	struct ring_buffer_pack *cpu_pack;
	int cpu, j;

	pack->nr_cpus = trace_buffer->cpus;
	pack->total_pages = 0;

	for_each_ring_buffer_pack(cpu_pack, cpu, pack) {
		struct ring_buffer_per_cpu *rb = trace_buffer->buffers[cpu];
		struct buffer_page *bpage = rb->head_page;
		unsigned long nr_pages = rb->nr_pages;
