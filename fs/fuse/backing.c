			     ...
			     struct file *file, unsigned int flags, umode_t mode)
{
	...
	*out = vfs_create(&init_user_ns, dir_fuse_inode->backing_inode,
...
int fuse_revalidate_backing(struct bpf_fuse_args *fa, int *out, struct inode *dir,
			    struct dentry *entry, unsigned int flags)
{
	struct fuse_dentry *fuse_dentry = get_fuse_dentry(entry);
	...
	if (unlikely(backing_entry->d_flags & DCACHE_OP_REVALIDATE)) {
		*out = backing_entry->d_op->d_revalidate(backing_entry, flags);
		return 0;
	}
	*out = 1;
	return 0;
...
int fuse_revalidate_finalize(struct bpf_fuse_args *fa, int *out, struct inode *dir,
			     struct dentry *entry, unsigned int flags)
{
	return 0;
}
...
int fuse_canonical_path_backing(struct bpf_fuse_args *fa, int *out, const struct path *path,
				struct path *canonical_path)
{
...
int fuse_canonical_path_finalize(struct bpf_fuse_args *fa, int *out,
				   const struct path *path,
				   struct path *canonical_path)
{
	return 0;
