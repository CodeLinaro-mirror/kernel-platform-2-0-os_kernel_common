...
{
	ssize_t ret = vfs_listxattr(get_fuse_dentry(dentry)->backing_path.dentry, list, size);
			 ...
			 unsigned int flags)
{
	...
	return fuse_rename_backing_common(olddir, oldent, newdir, newent,
					  fri->flags);
}
...
int fuse_statfs_backing(struct bpf_fuse_args *fa, struct dentry *dentry, struct kstatfs *buf)
{
	int err = 0;
	struct path backing_path;
...
void *fuse_statfs_finalize(struct bpf_fuse_args *fa, struct dentry *dentry, struct kstatfs *buf)
{
	struct fuse_statfs_out *fso = fa->out_args[0].value;
