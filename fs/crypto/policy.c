...
bool fscrypt_can_set_casefolding(const struct inode *inode)
{
	const struct fscrypt_info *ci;

	/*
	 * We should only allow enabling casefolding for a directory if it
	 * already has its key present
	 */
	ci = READ_ONCE(inode->i_crypt_info);
	if (ci == NULL)
		return false;

	return ci->ci_policy.version == FSCRYPT_POLICY_V2;
}
