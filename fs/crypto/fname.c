			...
			struct fscrypt_str *oname)
{
	...
	if (hash) {
		nokey_name.hash[0] = hash;
		nokey_name.hash[1] = minor_hash;
	} else {
		nokey_name.hash[0] = 0;
		nokey_name.hash[1] = 0;
	}
	...
	size += sizeof(nokey_name.hash);
	if (size > FSCRYPT_FNAME_UNDIGESTED_SIZE) {
		// compute sha256 of remaining name
		size += sizeof(nokey_name.sha256);
		memset(nokey_name.sha256, 0, 32);
	}

	printk("stringL:(%d) to_encode:%d\n", iname->len, size);
	oname->len = base64_encode((const u8 *)&nokey_name, size, oname->name);
	printk("encoded_to:%s(%d)\n", oname->name, oname->len);
	return 0;
}
			      ...
			      int lookup, struct fscrypt_name *fname)
{
	...
	fname->crypto_buf.name =
		kmalloc(max_t(size_t, FSCRYPT_FNAME_LONG_SIZE,
			      sizeof(struct fscrypt_nokey_name)),
			GFP_KERNEL);
	...
	if (ret < 0) {
		...
		fname->hash = n->hash[0];
		fname->minor_hash = n->hash[1];
		if (iname->len < FSCRYPT_FNAME_LONG_SIZE) {
			pr_info("Name does not require sha256!\n");
		}else {
			pr_info("Long name, will add hash!\n");
		}
		return 0;
	}
