...
static struct ext4_dir_entry_2 *dx_move_dirents(struct inode *dir, char *from, char *to,
...
{
	unsigned entry_space = dir->i_sb->s_blocksize - ext4_dir_rec_len(1, NULL) -
...
{
	unsigned entry_space = dir->i_sb->s_blocksize - ext4_dir_rec_len(0, dir);
		...
		unsigned char *name2, size_t len, bool quick)
{
	...
	if (quick)
		ret = utf8_strncasecmp_folded(um, name, decrypted_entry.name ? &decrypted : &entry);
	else
		ret = utf8_strncasecmp(um, name, decrypted_entry.name ? &decrypted : &entry);
	}
 ...
 */
static inline bool ext4_match(struct inode *parent,
			      ...
			      struct ext4_dir_entry_2 *de)
{
	...
	if (parent->i_sb->s_encoding && needs_casefold(parent)) {
		if (fname->cf_name.name) {
			...
			pr_info("DROSEN: Did not have cf_name!\n");
		}
		return !ext4_ci_compare(parent, fname->usr_fname, de->name, de->name_len,
					false);
	}
#endif
			...
			struct ext4_filename *fname)
{
	...
	if (IS_CASEFOLDED(dir) && IS_ENCRYPTED(dir)) {
		...
		hinfo.hash_version = DX_HASH_SIPHASH;
		hinfo.seed = NULL;
		ext4fs_dirhash(dir, fname_usr_name(fname), fname_len(fname), &hinfo);
			    ...
			    struct inode *inode, struct buffer_head *bh)
{
	...
	de->rec_len = ext4_rec_len_to_disk(blocksize - ext4_dir_rec_len(2, NULL),
					   blocksize);
	...
	if (IS_ENCRYPTED(dir) && IS_CASEFOLDED(dir))
		ext4fs_dirhash(dir, fname_usr_name(fname), fname_len(fname), &fname->hinfo);
	else
...
bool ext4_empty_dir(struct inode *inode)
{
	...
	sb = inode->i_sb;
	if (inode->i_size < ext4_dir_rec_len(1, NULL) + ext4_dir_rec_len(2, NULL)) {
			       ...
			       int force_reread)
{
	...
	if (le32_to_cpu(ent->de->inode) != ent->inode->i_ino ||
		...
		retval = ext4_delete_entry(handle, ent->dir, ent->de, ent->lblk, ent->bh);
		if (retval == -ENOENT) {
			retval = ext4_find_delete_entry(handle, ent->dir,
							&ent->dentry->d_name);
		}
