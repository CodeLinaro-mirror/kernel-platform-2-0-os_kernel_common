...
static void
trusty_sched_share_resources_release(struct trusty_sched_share_state *share_state)
{
...
trusty_sched_share_resources_allocate(struct trusty_sched_share_state *share_state)
{
	...
	dev_dbg(share_state->dev, "%s: sched_shared_vm=%llx  size=%d\n",
...
err_smc_std_call32:
	...
	trusty_sched_share_resources_release(sched_share_state);
err_resources_alloc:
...
void trusty_unregister_sched_share(struct trusty_sched_share_state *sched_share_state)
{
	...
	if (result != 0) {
		dev_err(sched_share_state->dev,
			"During unregister, trusty_reclaim_memory() failed!!\n");
		dev_err(sched_share_state->dev,
			"WARNING: leaking some allocated resources!!\n");
	}
