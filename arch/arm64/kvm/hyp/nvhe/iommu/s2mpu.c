...
{
	struct s2mpu_drv_data *data = iommu_drv_data(dev);
...
=======
>>>>>>> 26c5f6be152c (s2mpu dabt handler)
				    ...
				    u32 esr, size_t off)
{
	...
	mask = host_mmio_reg_access_mask(off, wnr);
	if (!mask)
		return false;
...
static int s2mpu_init(void *data, size_t szdata)
{
	struct mpt in_mpt;
	u32 *smpt;
	phys_addr_t pa;
...
const struct pkvm_iommu_ops pkvm_s2mpu_ops = (struct pkvm_iommu_ops){
	...
	.dev_data_size = sizeof(struct s2mpu_drv_data),
};
