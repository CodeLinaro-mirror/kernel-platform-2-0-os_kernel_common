...
static int ffa_guest_share_ranges(struct ffa_mem_region_addr_range *ranges,
				  ...
				  u64 vmid, struct ffa_mem_transfer *transfer,
				  struct arm_smccc_res *res)
{
	struct ffa_mem_region_addr_range *range;
			       ...
			       u64 vmid)
{
	...
	buf = hyp_buffers.tx;
	memcpy(buf, endp_buffers[vmid].tx, fraglen);
			    ...
			    u64 vmid)
{
	...
	memcpy(endp_buffers[vmid].rx, hyp_buffers.rx, copy_sz);
out_unlock:
	hyp_spin_unlock(&hyp_buffers.lock);
}
...
bool kvm_host_ffa_handler(struct kvm_cpu_context *ctxt, u32 func_id)
{
	...
	/* Memory management */
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_RXTX_UNMAP:
		do_ffa_rxtx_unmap(&res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_MEM_RECLAIM:
		do_ffa_mem_reclaim(&res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&res, ctxt, KVM_HOST_VMID);
		break;
	case FFA_VERSION:
		do_ffa_version(&res, ctxt);
		break;
	case FFA_PARTITION_INFO_GET:
		do_ffa_part_get(&res, ctxt, KVM_HOST_VMID);
		break;
	default:
		if (ffa_call_supported(func_id)) {
			handled = false;
