			    ...
			    u64 vmid)
{
	...
	if (vmid == KVM_HOST_VMID) {
		...
		pkvm_vcpu = PKVM_VCPU_FROM_CTXT(ctxt);
		ret = __pkvm_guest_share_hyp(pkvm_vcpu, tx, (u64 *)&tx_virt);
		if (ret) {
			ret = FFA_RET_INVALID_PARAMETERS;
			goto err_unmap;
		}

		ret = __pkvm_guest_share_hyp(pkvm_vcpu, rx, (u64 *)&rx_virt);
		if (ret) {
			ret = FFA_RET_INVALID_PARAMETERS;
			goto err_unshare_tx;
		}
				  ...
				  struct arm_smccc_res *res)
{
	...
	for (i = 0; i < nranges; i++) {
		...
		for (j = 0; j < range->pg_cnt; j++) {
			...
			ret = __pkvm_guest_share_ffa(vcpu, ipa, &pa);
			if (ret)
				goto unshare;
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	if (res.a0 == FFA_ERROR && res.a2 == FFA_RET_NO_MEMORY &&
	    res.a1 == ARM_EXCEPTION_HYP_REQ)
