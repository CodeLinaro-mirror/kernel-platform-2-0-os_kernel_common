...
static int guest_share_with_cb(struct kvm_cpu_context *ctxt, struct arm_smccc_res *res,
			       int(*cb)(struct pkvm_hyp_vcpu *, u64, u64 *),
			       u64 ipa, u64 *out_pa)
{
	int ret;
	u64 exit_code;
	struct pkvm_hyp_vcpu *pkvm_vcpu = PKVM_VCPU_FROM_CTXT(ctxt);
...
static void *ffa_alloc(size_t size, struct kvm_cpu_context *ctxt, struct arm_smccc_res *res)
{
	void *p;
	struct pkvm_hyp_vcpu *pkvm_vcpu = NULL;
	u64 exit_code;
	int ret;
			    ...
			    unsigned int vm_handle)
{
	...
	if (vm_handle == KVM_HOST_HANDLE) {
		...
		pkvm_vcpu = PKVM_VCPU_FROM_CTXT(ctxt);
		ret = guest_share_with_cb(ctxt, res, __pkvm_guest_share_hyp, tx, (u64 *)&tx_virt);
		if (ret)
			goto err_unmap;

		ret = guest_share_with_cb(ctxt, res, __pkvm_guest_share_hyp, rx, (u64 *)&rx_virt);
		if (ret)
			goto err_unshare_tx;

		ret = hyp_pin_shared_mem_from_guest(pkvm_vcpu, (void *)tx, tx_virt, tx_virt + 1);
		if (ret) {
			ret = FFA_RET_INVALID_PARAMETERS;
			goto err_unshare_rx;
		}

		ret = hyp_pin_shared_mem_from_guest(pkvm_vcpu, (void *)rx, rx_virt, rx_virt + 1);
		if (ret) {
			ret = FFA_RET_INVALID_PARAMETERS;
			goto err_unpin_tx;
		}
...
static int ffa_store_translation(struct ffa_mem_transfer *transfer,
				 u64 ipa, phys_addr_t pa, struct kvm_cpu_context *ctxt,
				 struct arm_smccc_res *res)
{
	struct ffa_translation *new_translation = ffa_alloc(sizeof(struct ffa_translation),
							    ctxt, res);
	if (!new_translation)
		return FFA_RET_NO_MEMORY;

	new_translation->ipa = ipa;
	new_translation->pa = pa;

	list_add(&new_translation->node, &transfer->translations);
				  ...
				  struct arm_smccc_res *res)
{
	...
	return 0;
unshare:
	ffa_guest_unshare_ranges(buf, mem_region_idx, ctxt, transfer);
	return ret;
}
					    ...
					    unsigned int vm_handle)
{
	...
	if (vm_handle) {
		...
		transfer = ffa_alloc(sizeof(struct ffa_mem_transfer), ctxt, res);
		if (!transfer) {
	...
	if (vm_handle) {
		...
		ret = ffa_guest_share_ranges(reg->constituents, nr_ranges, ctxt,
					     temp_reg, vm_handle, transfer, res);
		if (!ret) {
...
err_unshare:
	if (vm_handle)
		WARN_ON(ffa_guest_unshare_ranges(reg->constituents, nr_ranges, ctxt, transfer));
	else
...
bool kvm_host_ffa_handler(struct kvm_cpu_context *ctxt, u32 func_id)
{
	...
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_RXTX_UNMAP:
		do_ffa_rxtx_unmap(&res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_MEM_RECLAIM:
		do_ffa_mem_reclaim(&res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_MEM_FRAG_TX:
		do_ffa_mem_frag_tx(&res, ctxt, KVM_HOST_HANDLE);
		break;
	case FFA_VERSION:
		do_ffa_version(&res, ctxt);
		break;
	case FFA_PARTITION_INFO_GET:
		do_ffa_part_get(&res, ctxt, KVM_HOST_HANDLE);
		break;
	default:
...
bool kvm_guest_ffa_handler(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	...
	/* Memory management */
	case FFA_FN64_RXTX_MAP:
		do_ffa_rxtx_map(&res, ctxt, vm_handle);
		break;
	case FFA_RXTX_UNMAP:
	...
	case FFA_MEM_SHARE:
	case FFA_FN64_MEM_SHARE:
		do_ffa_mem_xfer(FFA_FN64_MEM_SHARE, &res, ctxt, vm_handle);
		break;
	case FFA_MEM_LEND:
	case FFA_FN64_MEM_LEND:
		do_ffa_mem_xfer(FFA_FN64_MEM_LEND, &res, ctxt, vm_handle);
		break;
	case FFA_MEM_RECLAIM:
