					   ...
					   enum pkvm_page_state desired)
{
	...
	if (tx->completer.id == PKVM_ID_HYP)
		checked_tx->completer_addr = (u64)__hyp_va(checked_tx->completer_addr);
		      ...
		      const struct pkvm_checked_mem_transition *checked_tx)
{
	...
	case PKVM_ID_FFA:
		 ...
		 * nothing to do here.
		 */
		tx->completer.ffa.completer_addr = checked_tx->completer_addr;
		ret = 0;
		break;
	case PKVM_ID_GUEST:
				  ...
				  void *hyp_va_from, void *hyp_va_to)
{
	...
	ret = __guest_check_page_state_range(vm, guest_ipa_from_aligned, size,
					     PKVM_PAGE_SHARED_OWNED);
