...
static int host_stage2_unmap_dev_all(void)
{
	...
	return host_stage2_unmap_dev_locked(addr, BIT(host_kvm.pgt.ia_bits) - addr);
}
...
static int host_stage2_idmap(u64 addr)
{
	...
	host_lock_component();
	/*
	 * Adjust aginst IOMMU devices first. host_stage2_adjust_range() should
	 * be last for proper alignment.
	 */
	if (!is_memory) {
...
void handle_host_mem_abort(struct kvm_cpu_context *host_ctxt)
{
	...
	if (is_dabt(esr) && !addr_is_memory(addr)) {
		if (pkvm_iommu_host_dabt_handler(host_ctxt, esr, addr)) {
			kvm_skip_host_instr();
			ret = 0;
		}
	}
