...
int __pkvm_iommu_driver_init(int id, void *data, size_t size)
{
	...
	if (IS_ENABLED(CONFIG_KVM_S2MPU)) {
		if (id == PKVM_IOMMU_DRIVER_S2MPU) {
			ret = __pkvm_register_iommu_driver(&pkvm_s2mpu_driver);
			if (ret)
				return ret;
		}
		if (id == PKVM_IOMMU_DRIVER_SYSMMU_SYNC) {
			ret = __pkvm_register_iommu_driver(&pkvm_sysmmu_sync_driver);
			if (ret)
				return ret;
		}
	}
