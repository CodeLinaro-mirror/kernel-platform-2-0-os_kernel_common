...
pub struct RawVacantEntry<'a, K, V> {
    ...
    /// The field that will contain a pointer to the new node if we insert one.
    ///
    /// This pointer is non-null, but it may point at a null pointer.
...
impl<'a, K, V> RawVacantEntry<'a, K, V> {
    ...
    /// [`RBTree`].
    pub fn insert(self, node: RBTreeNode<K, V>) -> &'a mut V {
